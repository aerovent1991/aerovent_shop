...
import { EwsCard } from '../components/ui/EwsCard';
import { FilterCheckbox } from '../components/ui/FilterCheckbox';
import type { EWS } from '../data/interface';

interface FiltersData {
  priceRange: { min: number; max: number };
  productionStatuses: Array<'inProduction' | 'discontinued' | 'madeToOrder'>;
}
...
export default function EwsCatalog() {
  const [searchQuery, setSearchQuery] = useState('');
  const [priceRange, setPriceRange] = useState<[number, number]>([0, 100000]);
  const [selectedStatuses, setSelectedStatuses] = useState<Array<'inProduction' | 'discontinued' | 'madeToOrder'>>([
    'inProduction'
  ]);
  const [sortOrder, setSortOrder] = useState<'asc' | 'desc' | null>(null);
  const [isMobileFiltersOpen, setIsMobileFiltersOpen] = useState(false);

  const [ewsList, setEwsList] = useState<EWS[]>([]);
  const [filteredEws, setFilteredEws] = useState<EWS[]>([]);
  ...
  const fetchEws = useCallback(async (page: number = 1) => {
    try {
      setLoading(true);
      ...
      const response = await fetch(`/api/ews?${params}`);
      const data = await response.json();

      if (data.success) {
        const list: EWS[] = data.data;
        setEwsList(list);
      ...
      console.error('Помилка завантаження EWS:', error);
    } finally {
      setLoading(false);
    }
  }, [searchQuery, selectedStatuses, priceRange, sortOrder]);

  const applyClientSort = useCallback((list: EWS[] = ewsList) => {
    let sorted = [...list];
    if (sortOrder === 'asc') {
  ...
  }, [searchQuery, selectedStatuses, priceRange]);

  useEffect(() => {
    applyClientSort();
  }, [sortOrder, ewsList]);

  const toggleStatus = (status: 'inProduction' | 'discontinued' | 'madeToOrder') => {
